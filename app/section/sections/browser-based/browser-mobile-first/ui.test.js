...
const pageSetup = async (t, responseStatus = 200, responseBody = {}) => {
  ...
  await t.navigateTo('http://localhost:1234/solution/S100000-001/dashboard/browser-based/section/browser-mobile-first');
};
...
test('should render all the advice of the section', async (t) => {
  ...
  const sectionManifest = new ManifestProvider().getSectionManifest({ dashboardId: 'browser-based', sectionId: 'browser-mobile-first' });
...
test('should goto anchor when clicking the mobile first design summary error link', async (t) => {
  ...
  await t
    ...
    .expect(getLocation()).contains('/S100000-001/dashboard/browser-based/section/browser-mobile-first#mobile-first-design');
});
